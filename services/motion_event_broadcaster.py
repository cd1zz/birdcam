...
class MotionEventBroadcaster:
    ...
    def __init__(self, cross_trigger_enabled: bool = True, trigger_timeout: float = 5.0):
        ...
        print(f"🎯 MotionEventBroadcaster initialized:")
        print(f"   🔗 Cross-camera triggering: {'enabled' if cross_trigger_enabled else 'disabled'}")
        print(f"   ⏱️  Trigger timeout: {trigger_timeout}s")
    ...
    def register_camera(self, camera_id: int, motion_callback: Callable[[MotionEvent], None]):
        ...
        with self._lock:
            ...
            print(f"📷 Camera {camera_id} registered with motion broadcaster")
    ...
    def unregister_camera(self, camera_id: int):
        ...
        with self._lock:
            if camera_id in self._motion_listeners:
                ...
                print(f"📷 Camera {camera_id} unregistered from motion broadcaster")
    ...
    def report_motion(self, camera_id: int, confidence: float = 1.0, location: Optional[tuple] = None):
        ...
        with self._lock:
            ...
            print(f"🎯 Motion detected on camera {camera_id} (confidence: {confidence:.2f})")
            ...
            if self.cross_trigger_enabled:
                ...
                for listener_camera_id, callback in self._motion_listeners.items():
                    try:
                        ...
                        print(f"❌ Error triggering camera {listener_camera_id}: {e}")
                
                if triggered_cameras:
                    print(f"🎬 Triggered recording on cameras: {triggered_cameras}")
    ...
    def set_cross_trigger_enabled(self, enabled: bool):
        ...
        with self._lock:
            ...
            print(f"🔗 Cross-camera triggering {'enabled' if enabled else 'disabled'}")
    
    def set_trigger_timeout(self, timeout: float):
        """Set the trigger timeout duration"""
        with self._lock:
            self.trigger_timeout = timeout
            print(f"⏱️  Trigger timeout set to {timeout}s")
